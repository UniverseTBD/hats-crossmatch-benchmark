...
```bash
uv run python -m benchmarks.cli sweep \
  --catalog-a sdss --catalog-b plasticc \
...
```bash
uv run python -m benchmarks.cli stream \
  --catalog-a sdss --catalog-b plasticc \
  --radius 1.0 --n-neighbors 1 \
  --output console json
```
...
```bash
uv run python -m benchmarks.cli stream-hf \
...
```bash
uv run python -m benchmarks.cli suite
```

Use `--mode stream` to run the suite in streaming mode:

```bash
uv run python -m benchmarks.cli suite --mode stream
```

### Run S3 benchmark suite

```bash
uv run python -m benchmarks.cli s3-suite
```

### Run stream-hf benchmark suite

```bash
uv run python -m benchmarks.cli stream-hf-suite
```

### List available catalogs

```bash
uv run python -m benchmarks.cli list
```
